Shader "Class2/Transparent"
{
    ...
    Subshader
    {
        ...
        half4 TransparentFragment(Varyings i) : SV_Target
        {
            ...
            float3 normalWS = normalize(float3(dot(i.T2W0.xyz, normalTS), dot(i.T2W1.xyz, normalTS),
                                               dot(i.T2W2.xyz, normalTS))) * _BumpIntensity;
            //Lighting
            Light mainLight = GetMainLight();
            half3 lightDir = mainLight.direction;
            half lambert = max(0.0, dot(normalWS, lightDir));
            return half4(lambert, lambert, lambert, pow(lambert, _TransparentThreshold));
        }
        ENDHLSL
